...
function TicketDetailPage({ ticket, onBack }: TicketDetailPageProps) {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const formattedMessages = ticketMessages.map(msg => ({
        ...msg,
        isAdmin: (msg.sender === "admin") ? true : false,
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const result = await addMessage(ticket.id, newMessage.trim());
      if (result) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-4 mb-6">
        ...
        <span
          className={`ml-auto px-3 py-1 rounded-full text-xs font-medium ${ticket.status === 'open'
      ...
      <div className={`flex-1 bg-gray-800/50 rounded-lg border border-gray-700 flex flex-col ${ticket.status === 'closed' ? 'opacity-75' : ''}`}>
              ...
              {messages.map((message) => (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <div
                      className={`max-w-[80%] rounded-lg p-4 ${message.isAdmin
